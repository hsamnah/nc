(function () {
    ...
    window['jsMU']['isCompatible'] = isCompatible;

    function isArray(myArray) {
        return myArray.constructor.toString().indexOf('Array') > 1;
    }
    window['jsMU']['isArray'] = isArray;
    ...
    window['jsMU']['preventDefault'] = preventDefault;
    ...
    window['jsMU']['stopPropagation'] = stopPropagation;
    ...
    window['jsMU']['getEventObject'] = getEventObject;
    ...
    window['jsMU']['addLoadEvent'] = addLoadEvent;
    ...
    window['jsMU']['removeEvent'] = removeEvent;
    ...
    window['jsMU']['getElementsByClassName'] = getElementsByClassName;
    ...
    window['jsMU']['toggleDisplay'] = toggleDisplay;
    ...
    window['jsMU']['insertAfter'] = insertAfter;
    ...
    function domMgmt() {
        ...
        this.removeCurentNode = function (elem) {
            var elem2Remove = (!jsMU.$(elem)) ? elem : jsMU.$(elem);
            if (elem2Remove != null
    ...
    window['jsMU']['domMgmt'] = new domMgmt();
    ...
    window['jsMU']['prependChild'] = prependChild;

    function objPos() {
        this.setObjPos = function (obj, relativeObj) {
            var f = jsMU.DomOopObjects.$(relativeObj);
            var x = 0;
            var y = 0;
            while (f) {
                x += f.offsetLeft;
    ...
    window['jsMU']['getBrowserWindowSize'] = new getBrowserWindowSize();
    ...
    window['jsMU']['getPointerPositionInDocument'] = getPointerPositionInDocument;
    ...
    window['jsMU']['getMyBoundingClientRect'] = getMyBoundingClientRect;
    ...
    window['jsMU']['getKeyPressed'] = getKeyPressed;
    ...
    window['jsMU']['setStyle'] = setStyleById;
    window['jsMU']['setStyleById'] = setStyleById;
    ...
    window['jsMU']['setStylesByClassName'] = setStylesByClassName;
    ...
    window['jsMU']['setStylesByTagName'] = setStylesByTagName;
    ...
    window['jsMU']['getWindowSize'] = getWindowSize;

    // Canvas functions will now follow:
    function degree2rad(degree) {
        return Math.PI * degree / 180;
    };
    window['jsMU']['degree2rad'] = new degree2rad();
    ...
    window['jsMU']['xmlRequest'] = new xmlRequest();
    ...
    function colorSwatch(eventSrc) {
        ...
        this.colorPalette = function (target, selection) {
            ...
            jsMU.Accessor(esrc, 'e', function (x) { return typeof x == 'object'; });
            esrc.sete(t);
            ...
            for (var i = 0; i < Spectrum.length; i++) {
                for (var j = 0; j < Spectrum.length; j++) {
                    ...
                    for (var k = 0; k < Spectrum.length; k++) {
                        ...
                        jsMU.addEvent(ccell, 'mouseover', function (W3CEvent) {
                            var e = event.srcElement;
                            var target = jsMU.$('clrIO');
                            target.value = e.style.backgroundColor;
                            td1.style.backgroundColor = e.style.backgroundColor;
                        })
                        jsMU.addEvent(ccell, 'mouseout', function (W3CEvent) {
                            var target = jsMU.$('clrIO');
                            target.value = '';
                            td1.style.backgroundColor = '';
                        });
                        jsMU.addEvent(ccell, 'click', function (W3CEvent) {
                            var e1 = event.srcElement;
                            jsMU.preventDefault(ccell);
                            jsMU.stopPropagation(ccell);
                            ...
                            if (esrc.gete().nodeName === 'INPUT') {
                                ...
                                jsMU.domMgmt.removeChildren(jsMU.$('clrSwatch'));
                                jsMU.domMgmt.removeCurentNode(jsMU.$('clrSwatch'));
                            }
            ...
            container.style.top = jsMU.objPos.returnObjTop(esrc.gete()) + jsMU.objPos.returnObjHeight(esrc.gete()) + 'px';
            container.style.left = jsMU.objPos.returnObjLeft(esrc.gete()) + 'px';
            jsMU.addEvent(window, 'resize', function (W3CEvent) {
                container.style.top = jsMU.objPos.returnObjTop(esrc.gete()) + jsMU.objPos.returnObjHeight(esrc.gete()) + 'px';
                container.style.left = jsMU.objPos.returnObjLeft(esrc.gete()) + 'px';
            });
    ...
    if (!window.jsMU) { window['jsMU'] = {}; }
    window['jsMU']['colorSwatch'] = new colorSwatch();
    ...
    if (!window.jsMU) { window['jsMU'] = {}; }
    window['jsMU']['newWindow'] = new newWindow();
    ...
    window['jsMU']['asyncImport'] = new asyncImport();
})();
var containerElem=null, hfgm=null, details=null;
jsMU.addLoadEvent(function (W3CEvent) {
    details = jsMU.getElementsByClassName('details_cls', 'SPAN');
    for (var i = 0; i < details.length; i++) {
        jsMU.addEvent(details[i], 'click', function (W3CEvent) {
            jsMU.asyncImport.init('http://maps.googleapis.com/maps/api/js?callback=initialize&key=AIzaSyBQd7QvR8I53IiPEz14eVQ_Yx3-Oeztxds&v=3.exp&sensor=false', 'js');
        })
    }
}, false);
jsMU.addEvent(window, 'load', function (W3CEvent) {
});
jsMU.addLoadEvent(function (W3CEvent) {
}, true);
...
function initialize() {
    ...
    containerElem = jsMU.getElementsByClassName('gmap_cls', 'DIV');
    hfgm = jsMU.getElementsByClassName('hfg_cls', 'INPUT');

    for (var i = 0; i < containerElem.length; i++) {
        jsMU.asyncImport.getLatLongFromAdd(hfgm[i].value, geocoder, function (Addy,data) {
            ...
            var map = new google.maps.Map(jsMU.$('gmap_'), mapProp);
